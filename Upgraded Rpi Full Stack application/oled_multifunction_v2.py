...
screen_pages = ["1", "3", "S"] # This array holds the sensor ID (numeral) or function code (S for "disk Space").
...
def get_disk_space():
    ...
    local_time = arrow.get(current_time, "HH:mm:ss").to(timezone)
    local_time_formatted = local_time.format('HH:mm:ss')

    free_space = os.statvfs('/')
    free_space_in_Gbytes = (free_space.f_bavail * free_space.f_frsize) / (1024*1024*1024)
    line1 = local_time_formatted
    line2 = str(round(free_space_in_Gbytes,2)) + " GB" 
    line3 = ""
    line4 = ""

    return [line1, line2, line3, line3]
