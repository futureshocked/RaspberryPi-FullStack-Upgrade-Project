...
1. WHAT IT DOES
Takes a reading from a BME280 sensor and records the values in an SQLite3 database using a Raspberry Pi.
...
3. ORIGINAL WORK
Raspberry Full Stack 2015, 2019, and Raspberry Pi Full Stack Upgrade Project, Peter Dalmaris

4. HARDWARE
BME280, HC12, OLED module. See course for schematic diagrams.
'''
...
def text_alert(device_id, temp, hum, pres):
    account_sid = os.environ["TWILIO_ACCOUNT_SID"]
    auth_token = os.environ["TWILIO_AUTH_TOKEN"]
    my_twilio_phone_number = os.environ["TWILIO_PHONE_NUMBER"]
    receive_phone_number = os.environ["MY_PHONE_NUMBER"]
    deg_sign = u"\N{DEGREE SIGN}"
    report = f'Device {device_id} reported a temperature of {temp}{deg_sign}C, {hum}% humidity  and {pres} hPa..'
...
def log_values(sensor_id, temp, hum, pres):
        ...
        curs.execute("""INSERT INTO pressures values(datetime(CURRENT_TIMESTAMP, 'localtime'), (?), (?))""", (sensor_id,pres))     #This will store the new record at UTC
        conn.commit()
        conn.close()
        ...
        row = [strftime("%Y-%m-%d %H:%M:%S", gmtime()),sensor_id,round(temp,2),round(hum,2),round(pres,2)]
        sheet.append_row(row)
        #text_alert(sensor_id, temp, hum, pres)
...
if humidity is not None and temperature is not None and pressure is not None:
	log_values("1", temperature, humidity, pressure)
else:
