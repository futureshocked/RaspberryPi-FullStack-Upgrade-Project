...
def log_values(sensor_id, temp, hum, pres):
        ...
        curs.execute("INSERT INTO pressures values(datetime(CURRENT_TIMESTAMP, 'localtime'), ?, ?)", (sensor_id,float(pres)))
        conn.commit()
        conn.close()
        ...
        row = [strftime("%Y-%m-%d %H:%M:%S", gmtime()),sensor_id,temp,hum,pres]  # Not using round because temp and hum are already strings
