...
const byte verify_pin = 32;

// This part of the code is taken from https://forum.arduino.cc/t/serial-input-basics-updated/382007
const byte numChars = 50;      // Use this buffer to verify that message was received correctly
...
void IRAM_ATTR onLEDTimer() {
  timerAlarmDisable(LED_off_timer);
...
void setup() {
  ...
  // timerAlarmWrite(Transmit_timer, 30000000, true);
  ...
  // Setup the LED off message timer
  LED_off_timer = timerBegin(1, 80, true);
  timerAttachInterrupt(LED_off_timer, &onLEDTimer, true);
  timerAlarmWrite(LED_off_timer, 150000, false);  // Repeat set to false. Remember to restart the timer in code.
  timerAlarmEnable(LED_off_timer);                //Just Enable
}
...
void take_sensor_reading_and_prep_message() {
  ...
  hum = bme.hum();
  pres = bme.pres();
  message = String(millis_message_stamp) +
                        "," + String(temp, 2) +
                        "," + String(hum, 2) +
                        "," + String(pres, 2) +
                        "," + String(this_device_id) +
                        ",999";  // ! is the end character
...
void verify_transmission() {
  if (newData == true) {
    Serial.println("-----------------");
    Serial.print("Sent: ");
    Serial.println(last_message_sent);
    Serial.print("Recv: ");
    Serial.println(receivedChars);
    Serial.println("-----------------");
    if (strcmp(last_message_sent, receivedChars) == 0)
      digitalWrite(verify_pin, HIGH);
    timerWrite(LED_off_timer, 0);  //reset timer (feed watchdog)
    timerAlarmEnable(LED_off_timer);
  }
  newData = false;
  // digitalWrite(verify_pin, LOW);
}
